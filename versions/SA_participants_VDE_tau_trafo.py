...
class SlottedAloha__participants_VDE_tau_trafo(SlottedAloha.SlottedAloha_Class):
    """
    ...
    def filterPowerValue(self, P_new, inputs):
        ...
        if self.P_old > P_new:
            if self.S > TRAFO_LIMIT and self.getAtt('Vm', inputs) > (NORM_VOLTAGE * 0.88):
                difference = (self.P_old - P_new) * 0.368
            else:
    ...
    def charging(self, inputs):
        ...
        if P_new > 0:
            ...
            self.P_out = self.filterPowerValue(0.0, inputs)
            self.chargingFLAG = False
            self.arriverFlag = False
